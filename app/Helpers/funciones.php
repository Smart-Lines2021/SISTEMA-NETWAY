...
function traslado($xml,$concepto){
	...
	function cantidadNotificaciones(){
		...
		$servicios=ServicioVehiculo::get();
		...
		//Contamos las notificaciones de los servicios
		foreach ($servicios as $servicio) {
			$fechaServicio = $servicio->fecha;
			//Obtenemos la fecha de hace 3 dias
			$fechaServicio =  date('Y-m-d',strtotime($fechaServicio."- 3 days"));
			if ($fechaServicio == $fecha) { //Si la fecha de hoy coincide con la fecha del vencimiento de la poliza 3 dias antes
				$notificacion++;
			}
		}
		return $notificacion;
	}
	function mensajesNotificaciones(){
		$mensaje=array();
		$fecha=date('Y-m-d'); //Obtenemos la fecha actual
		$polizas=PolizaVehiculo::where('activo','=',1)->get();
		$servicios=ServicioVehiculo::get();
		$tiempoNotificacion=time();
		$tiempo=date("H", $tiempoNotificacion);
		foreach ($polizas as $poliza) {
			$fechaPoliza = $poliza->vigencia_poliza;
			//Obtenemos la fecha de hace 3 dias
			$fechaPoliza =  date('Y-m-d',strtotime($fechaPoliza."- 3 days"));
		...
		foreach ($servicios as $servicio) {
			...
			$fechaServicio =  date('Y-m-d',strtotime($fechaServicio."- 3 days"));
			if ($fechaServicio == $fecha) { //Si la fecha de hoy coincide con la fecha del vencimiento de la poliza 3 dias antes
				$mensaje[]='Debe llevarse el servicio a la flotilla '.$servicio->vehiculo->nombre.' dentro de 3 dias'.'°'.$tiempo.'°'.'rh.vehiculos.show'.'°'.Crypt::encryptString($servicio->vehiculo_id);
			}
		}
		return $mensaje;
	}
