<div class="modal fade" aria-hidden="true" id="modal-edit-{{$estado->id}}" tabindex="-1" role="dialog" tabindex="-1"
  aria-hidden="true">
  <form method="POST" action="{{route('admin.estados.update',Crypt::encryptString($estado->id))}}">
    @method('PUT')
    @csrf
      ...
      <div class="modal-body">
       <div class="form-group">
        ...
        <div class="form-group">
          ...
          value="{{old('variable',$estado->variable)}}">
        </div>
        </div>
        <div class="col-6">
        <div class="form-group">
          ...
          value="{{old('renapo',$estado->renapo)}}">
        </div>
      </div>
      <div class="col-6">
        <div class="form-group">
          ...
          value="{{old('clave_curp',$estado->clave_curp)}}">
        </div>
      </div>
      <div class="col-6">
        <div class="form-group">
          ...
          minlegth="2" maxlength="3"
          title="Solo se permiten letras. Tamaño mínimo: 2. Tamaño máximo: 3"
          value="{{old('tres_digitos',$estado->tres_digitos)}}">
        </div>
      </div>
    </div> {{-- cierra el row --}}
    <div class="form-group">
      <label for="pais_id">Pais:</label>
          ...
          <option {{ old('pais_id') == $estado->pais->id ? "selected" : "" }} value="{{$estado->pais->id}}"> {{$estado->pais->nombre}}</option>
        ...
        <div class="modal-footer">
          ...
          <button class="btn btn-primary">Editar Estado</button>
        </div>
      </div>
    </div>
