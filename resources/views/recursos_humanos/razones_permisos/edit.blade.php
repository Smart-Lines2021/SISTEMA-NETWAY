<div class="modal fade" aria-hidden="true" id="modal-edit-{{$razonPermiso->id}}" tabindex="-1" role="dialog" tabindex="-1"
  aria-hidden="true">
  <form method="POST" action="{{route('rh.razones_permisos.update',Crypt::encryptString($razonPermiso->id))}}">
    @method('PUT')
    @csrf
    <div class="modal-dialog" role="document">
      <div class="modal-content">
       <div class="modal-header">
        <h4 class="modal-title">Editar Razón del Permiso</h4>
      ...
      <div class="form-group">
          ...
          value="{{old('duracion',$razonPermiso->duracion)}}">
        </div>
       <div class="form-group">
          ...
          {{old('descripcion',$razonPermiso->descripcion)}}
          </textarea>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button>
          <button class="btn btn-primary">Editar Razón del Permiso</button>
        </div>
      </div>
    </div>
